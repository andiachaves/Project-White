...
real b0site[nSite];
real b0month_site[nMonth];


// level 3 random intercept
for (s in 1:nSite) b0site[s] = b0 + site_raw[s] ;

// level 2 random intercept
for (m in 1:nMonth)  b0month_site[m] = b0site[Site[m]] + month_raw[m];



for (i in 1:nWP) {

RealDisease[i] =  Environment[i] * parEnvironment + b0month_site[Month[i]]+eps[i];
...
site_raw ~ normal(0, tauSite);
  ...
  //tauMonth ~ cauchy(0,2.5);
  //tauSite ~ cauchy(0,2.5);
...
# FITTED PREDICTED from linear predictor
...
# SIMULATED from generated quantities
modelPredictions <- NULL
