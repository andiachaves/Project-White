...
// Number of observations Level-1 (integer)

int<lower=0> nWP;                      // number of White Plague cases, int=integer  and non negative >0 int<lower=1> nEnvi;                    // number of environmental variables
...
//PLEASE DO NOT DELETE THESE ONES, I KNOW WE ARE NOT USING THEM RIGHT NOW
//int<lower=1> nYear; 
//int<lower=1,upper=4> Year[nWP];  // year of each observation
//int<lower=1> nSeason; 
//int<lower=1,upper=4> Season[nWP];  // season of each observation
...
//vector[nWP] Season;
//vector[nWP] Month;
//vector[nWP] Temperature;
//vector[nWP] Turbidity;
//vector [nWP] Depth;
...
//real parSeason;   // Season-level
//real parMonth;   // Month-level
//real parDepth;   // Depth-level
//real parTurbidity;   // Turbidity-level
...
//Level-3 Month nested within Site
...
//for( i in 1:nWP ) {
//RealDisease[i]  = (b0  + Environment[i] * parEnvironment + (site_raw[Site[i]]/month_raw[Month[i]])+ //eps[i]);


// Varying intercepts definition
 // Level-3 (level-3 random intercepts)
for (k in 1:nSite) {
Site[k] <- b0  + site_raw[Site[i]];
}
// Level-2 (level-2 random intercepts)
for (j in 1:nMonth) {
Month[j] <- site_raw[Site[i]] + month_raw[Month[i]];
}
// Individual mean
for (i in 1:nWP) {
RealDisease[i] <-  Environment[i] * parEnvironment + month_raw[Month[i]];
...
//parSeason      ~ normal(0, 100 );
//parMonth      ~ normal(0, 100 );
//parTemperature ~ normal(0, 100 );
//parDepth ~ normal(0, 100 );
//parTurbidity ~ normal(0, 100 );
...
//year_raw                ~ normal(0, tauYear);
//season_raw              ~ normal(0, tauSeason);
month_raw               ~ normal(0, tauMonth);
site_raw               ~ normal(0, tauSite);
// overdispersion
...
```{r}
save(stan.white, file =paste("data/2017.11.13a_Whiteplague_model_result.Rdata",sep=""))
```
