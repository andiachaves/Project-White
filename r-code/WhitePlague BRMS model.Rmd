...
#model as for GLMER without autocorrelation function
```{r}
stan.white <- brm(ill|trials(Ntot) ~ Depth_transect + Turb_sca + (1|Station_code/Month),
                                data=Whiteplague_workingdata,
                                family = binomial(link="logit"))
```

#model as for GLMER wit autocorrelation function
```{r}
stan.white <- brm(ill|trials(Ntot) ~ Depth_transect + Turb_sca + (1|Station_code/Month),
                                data=Whiteplague_workingdata,
                                family = binomial(link="logit"),
