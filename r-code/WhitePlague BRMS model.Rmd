...
```{r}
stan.white <- brm(ill|trials(Ntot) ~ Depth_tran_sca + Turb_sca + (1|Station_name/Month),
                                data=Whiteplague_workingdata,
                                family = binomial(link="logit"))
```

#model as E3 for GLMER wit autocorrelation function
```{r}
stan.white <- brm(ill|trials(Ntot) ~ Depth_tran_sca  + Turb_sca + (1|Station_name/Month),
                                data=Whiteplague_workingdata,
                                family = binomial(link="logit"),
...
```{r}
stan.white <- brm(ill|trials(Ntot) ~ Depth_transect + Turb_sca + (1|Season/Month:Station_code),
                                data=Whiteplague_workingdata,
                                family = binomial(link="logit"))
```
