...
for( i in 1:nWP ) {
RealDisease[i]  = (b0  + Environment[i] * parEnvironment + Month[i]*parMonth 
...
stan.test <- stan(model_code = model.Wplague, data = dataList, iter = 100, chains = 1,refresh=1,thin=1)
...
```{r}
save(stan.white, file =paste("data/2017.07.28_Whiteplague_model_result.Rdata",sep=""))
```
