...
#enviroselection
Environment= subset(Whiteplague_workingdata, select=c("Depth_tran_sca"))
...
for (i in 1:nWP) {
RealDisease[i] =  Environment[i] * parEnvironment + site_raw[Site[i]];

} //closes loop observations
//RealDisease =  b0 + Environment * parEnvironment+ site_raw[Site];
...
for (s in 1:nSite) site_raw[s] ~ normal(month_raw[Month[s]], tauSite);
for (m in 1:nMonth) month_raw[m] ~ normal(0, tauMonth);
...
```{r}
save(stan.white, file =paste("data/2017.11.14c_Whiteplague_model_result.Rdata",sep=""))
```
