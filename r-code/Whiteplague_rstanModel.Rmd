...
for( i in 1:nWP ) {
RealDisease[i]  = (b0  + Environment[i] * parEnvironment + month_raw[Month[i]]+ eps[i]);
...
```{r}
save(stan.white, file =paste("data/2017.10.10c_Whiteplague_model_result.Rdata",sep=""))
```
...
# Adding autocorelation with brms (Bayesian Regression Models using Stan)
p= autoregressive (AR) order
```{r}
temporal.autocorrelation <- brm(ill|trials(Ntot) ~ (1|Depth_transect),
...
#Save the model run results
```{r}
save(temporal.autocorrelation, file =paste("data/2017.10.13a_Whiteplague_model_result.Rdata",sep=""))
```
