...
## Data preparation
```{r}
df <- Whiteplague_workingdata[,c("Ntot","ill","Plague","Temp_sca","Turb_sca","Depth_tran_sca")]
...
Environment <- df[,c("Turb_sca","Temp_sca")]
```
...
Nchains=3
time <- proc.time()
stan.white <- stan(model_code = model.Wplague, data = dataList, iter = 2000, chains = Nchains,refresh=100)
...
```{r}
save(stan.white, file =paste("data/2018.01.18a_Whiteplague_model_result.Rdata",sep=""))
```
