...
for( i in 1:nWP ) {
RealDisease[i]  = b0  + Environment[i] * parEnvironment + year_raw[Year[i]] + (year_raw[Year[i]])^2 + (year_raw[Year[i]])^3 + eps[i];
...
```{r}
save(stan.white, file =paste("data/2017.08.30c_Whiteplague_model_result.Rdata",sep=""))
```
