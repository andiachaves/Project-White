...
for( i in 1:nWP ) {
RealDisease[i]  = b0  + Environment[i] * parEnvironment + year_raw[Year[i]] + season_raw[Season[i]]+ month_raw[Month[i]]+ (month_raw[Month[i]])^2 + (month_raw[Month[i]])^3 + eps[i];
...
```{r}
save(stan.white, file =paste("data/2017.08.30b_Whiteplague_model_result.Rdata",sep=""))
```
