...
dataList = list(
        ...
        nTemperature = length(unique(Temperature))
        )
```
...
int<lower=1,upper=13> Site[nWP];  // season of each observation
int<lower=1> nTemperature;               // Temperature
...
vector[nSeason] parSeason;   // Season-level
vector[nTemperature] parTemperature;   // temperature effect
...
for( i in 1:nWP ) {
RealDisease[i]  = (b0  + Environment[i] * parEnvironment +  Season[i] * parSeason+ parTemperature* Season[i]+ year_raw[Year[i]] + eps[i]);
...
```{r}
save(stan.white, file =paste("data/2017.08.31a_Whiteplague_model_result.Rdata",sep=""))
```
