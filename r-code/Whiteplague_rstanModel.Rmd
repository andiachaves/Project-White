...
Timepoint=Whiteplague_workingdata$Time_point
...
int<lower=1> nMonth; // number of Months sampled
int<lower=1> nSite;  // number of Sites sampled
int<lower=1> nYear;  // number of Sites sampled
int<lower=1> nTimepoint; // number of sampling dates (temporal autocorrelation)

// predictors
matrix[nWP, nEnvi] Environment;
int<lower=1> Month[nWP];
int<lower=1,upper=nSite> Site[nWP];// site id for each observation
int<lower=1> Year[nWP];
int<lower=1> Timepoint[nWP];
...
for (j in 1:nMonth) {
u[j] = month_raw[j]*sqrt(tauMonth);
}

for (j in 1:nSite) {
v[j] = site_raw[j]*sqrt(tauSite);
}

for (j in 1:nTimepoint) {
w[j] = timepoint_raw[j]*sqrt(tautimepoint);
}

// this loop integrates the time lag in days between sampling dates (auto-regressive structure)

for( i in 1:nWP ) {
y[i] = dnorm(y.mean[i],u.tau) 
d[i] = 365* (w[Timepoint[i]] - w[Timepoint[i-1]]); // 365 days in a year
mu[i] = pow(abs(rho1),d[i])*(step(rho1)-step(-rho1))
...
RealDisease[i]  = (nu[i] + mu[i]*(y[i-1]-nu[i-1]); // this is the full model equation
...
parMonth       ~ normal(0, 100 );
parYear        ~ normal(0, 100 );

//Hyperpriors
tauSite    ~ inv_gamma(0.001, 0.001);
tauMonth   ~ inv_gamma(0.001, 0.001);
//tauYear  ~ inv_gamma(0.001, 0.001);

//Random effects priors
site_raw  ~ normal(0,10);
month_raw ~ normal(0,10);
timepoint_raw ~ normal(0,10);
//year_raw ~ normal(0,10);
...
stan.test <- stan(model_code = model.Wplague, data = dataList, iter = 20, chains = 1,refresh=1,thin=1)
