...
for( i in 1:nWP ) {
mu[i] = u[Month[i]] + v[Site[i]];
RealDisease[i]  = (b0  + Environment[i] * parEnvironment + Month[i]*parMonth + Year[i]*parYear + mu[i]
                  );
}
}
...
```{r}
save(stan.white, file =paste("data/2017.07.30_Whiteplague_model_result.Rdata",sep=""))
```
