...
E5 <- glm(cbind(Ntot,ill) ~  Month + Turbidity + (1|Site/Depth) ,family="binomial"(link = "logit"))
summary(E5)

E6 <- glm(cbind(ill,Ntot-ill) ~ Depth + Turbidity + (1|Site*Month) ,family="binomial"(link = "logit"))
summary(E6)
...
simulationOutput <- simulateResiduals(fittedModel = E3, n = 250)
...
Site_index <- as.integer(Whiteplague_workingdata$Station_name)
Month_index=as.integer(Whiteplague_workingdata$Month_12)

G1 <- glmer2stan(cbind(ill,Ntot-ill) ~ Depth + Turbidity+ (1|Site_index) ,family="binomial")
