...
# load data & libraries
White <- read.csv("data/rawdata.csv", header = TRUE)
```
...
enviroselection = c(12:16) # to select environmental variables
```
...
RealDiseasePredictions [nWP]; //vector to store predictions
for (i in 1:nWP) {
#using Binomial rng= random number generator
RealDiseasePredictions[i] <binomial(TotalObservedcolonies[i],inv_logit(b0+ Environment[i] * parEnviro  + Cor[i] * parCor + mu[i]);
}
'
```
...
```{Predictive inference}
#//Predictive Inference
#DiseasePredictions[nWP]; //vector to store predictions
#for (i in 1:nWP) {
#using Binomial rng= random number generator
#RealDiseasePredictions[i] <binomial(TotalObservedcolonies[i],inv_logit(b0+ Environment[i] * #parEnviro  + Cor[i] * parCor + mu[i]));)
```
...
StanCat::stan_caterpillar(stan.season, pars = 'parEnvironment\\[.*\\]', pars_labels= c("DHW", "Chlorophyll", "Salinity", "Turbidity", "Temperature"))
