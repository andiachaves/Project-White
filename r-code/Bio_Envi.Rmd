...
enviroselection = c(13:16) # to select environmental variables
```
...
dataList = list (
        ...
        Environment = White[13:(13-1+length(enviroselection))], # Temp, Chlorophyll, Salinity, Turbidity
...
PrevalenceDiseasePredictions[nWP]; //vector to store predictions

for (i in 1:nWP) {

PrevalenceDiseasePredictions[i]<binomial_rng(TotalObservedcolonies[i],inv_logit(b0  + Environment[i] * parEnviro  + Grapes[i] * parGrapes + mu[i]));)}
