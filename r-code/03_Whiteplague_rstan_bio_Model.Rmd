...
## Data preparation
```{r}
df <- data.frame(Whiteplague_workingdata_Annual[,c("Station_code","Year","Season","Depth", "Ntot","ill","Plague","Chl_sca","Oxy_sca","Sal_sca","Turb_sca","Temp_sca","DHW_sca", "Sand", "Coral", "Sponges","Macroalgae", "Cyanobacteria")])
...
df$Depth    <- as.factor(as.numeric(Whiteplague_workingdata_Annual$Depth))
df$Obsid   <- as.factor(1:nrow(df))
df$DepthYear <- as.factor(as.numeric(as.factor(paste(df$Year,df$Depth,sep="-"))))
...
df$DepthYear_index=as.integer(as.numeric(as.factor(paste(df$Year,df$Depth,sep="-"))))

#Environment <- df[,c("Chl_sca","Oxy_sca","Sal_sca","Turb_sca","Temp_sca","DHW_sca","Sand", "Coral", "Sponges","Macroalgae", "Cyanobacteria")]
...
# Environment <- df[,c("Chl_sca","Chl_sca2","Oxy_sca","Oxy_sca2","Sal_sca","Sal_sca2","Turb_sca","Turb_sca2","Temp_sca","Temp_sca2",
#                      "DHW_sca","DHW_sca2","Sand", "Sand2","Coral","Coral2", "Sponges","Sponges2","Macroalgae", "Macroalgae2","Cyanobacteria","Cyanobacteria2")]


# Environment <- df[,c("Chl_sca","Chl_sca2","Oxy_sca","Oxy_sca2","Sal_sca","Sal_sca2","Turb_sca","Turb_sca2","Temp_sca","Temp_sca2", "DHW_sca","DHW_sca2",
#                      "Sand","Coral","Sponges","Macroalgae", "Cyanobacteria")]


Environment <- df[,c("Chl_sca","Chl_sca2","Oxy_sca","Oxy_sca2","Sal_sca","Sal_sca2","Turb_sca","Temp_sca","Temp_sca2", "DHW_sca","DHW_sca2",
                     "Sand","Coral","Sponges","Macroalgae", "Cyanobacteria")]
...
#Environment <- df[,c("Chl_sca","Oxy_sca","Sal_sca","Turb_sca","Temp_sca","DHW_sca","Sand", "Coral", "Sponges","Macroalgae", "Cyanobacteria","Year1","Year2","Year3")]
```
...
dataList = list(
  ...
  Depth = df$Depth_index,
  nDepth =length(unique(df$Depth_index),
                 
  DepthYear = df$DepthYear_index,
  nDepthYear =length(unique(df$DepthYear_index))
         )        
)
```
...
```{r}
save(stan.white, file =paste("data/Extraction_diagnosis/2018.02.28_Whiteplague_model_result.Rdata",sep=""))
```
