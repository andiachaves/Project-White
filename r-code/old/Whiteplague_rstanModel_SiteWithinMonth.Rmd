...
//Level-2 Month nested within Site
...
real b0month[nMonth];
real b0site[nMonth];
//real b0month_site[nSite];


// level 3 random intercept
//for (m in 1:nMonth) b0month[m] = b0 + month_raw[m] ;

// level 2 random intercept
//for (s in 1:nSite)  b0month_site[s] = b0month[Month[s]] + site_raw[s];
...
//RealDisease[i] =  Environment[i] * parEnvironment + b0month_site[Site[i]]+eps[i];
...
month_raw ~ normal(0, 100);
site_raw ~ normal(0, 100);
...
time <- proc.time()
set.seed(2017)
stan.white <- stan(model_code = model.Wplague, data = dataList, iter = 3000, chains = 4,refresh=1)
