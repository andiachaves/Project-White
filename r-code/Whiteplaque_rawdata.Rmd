...
White <- read.csv("data/Rawdata_WP_No_average.csv", header = TRUE)
ls(White)
dim(White)
head(White)
```
...
SIMPLE LINEAR REGRESSION PLOTS - ENVIRONMENTAL VARIABLES VS WHITE PLAGUE PREVALENCE

```{r}
#Chlorophyll vs WP Prevalence
p72 <- ggplot(White, aes(x = White_Plague_Pervalence, y = Chlorophyll)) + scale_y_continuous(name = "Chlorophyll"~("mg m"^{-3})) + scale_x_continuous(name = "WP Prevalence (diseased/total)") + geom_point(shape=1) + geom_smooth() #using method loess
p72

#Conductivity
p73 <- ggplot(White, aes(x = White_Plague_Pervalence, y = Conductivity)) +scale_y_continuous(name = "Conductivity"~("COS m"^{-1})) + scale_x_continuous(name = "WP Prevalence (diseased/total)") + geom_point(shape=1) + geom_smooth() #using method loess
p73
...
VARIABLES INTERACTIONS TESTING- Likelihood Ratio test using ANOVA/ANCOVA 
