...
g=function(alpha, gamma,c, L){
	...
	#New model:
	mu = alpha * exp(gamma - kappa*L) / 1 + (exp(gamma - kappa*L)
...
##Original: get_data=function(alpha, gamma,c, sigma){
get_data=function(alpha, gamma, kappa, sigma){
	set.seed(4)
	par(mfrow=c(1,1))
	L=sort(runif(50,min=12, max = 100))
	mu = g(alpha=alpha, gamma=gamma, kappa = kappa, L=L)
	plot(L,mu, typ="l")
...
plot_density = function(p,v1,v2, param, burnin, n.iter){
	...
	abline(v=v1,col="red", lwd=2)  #red is your true value
	abline(v=v2, col="blue", lwd=2)  #blue is from your least squares
	abline(v=median(p[burnin:n.iter]), col="orange", lwd=2)   #orange is from MCMC
			###So in the plots the orange tends to be closest to the true value.
}
