...
source("Model2_inits.R")
n.chains = length(inits2)
...
jm1=jags.model("Day7_model2_SOLUTIONS.R",
               data=scaling_data,
               inits=inits2,
               n.chains=n.chains,
               n.adapt=n.adapt)
...
autocorr.plot(window(zc1, start=st+bi, end=en, thin=5)[,c("deviance")],lag.max=30)
