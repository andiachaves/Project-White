...
scaling_data = list(N=256, Nvars = 2, R=structure(.Data=c(1,0,0,1),.Dim=c(2,2)),
	LogD = LogD, Y = Y)
...
jm1=jags.model("Day7_model3_SOLUTIONS.R",
               data=scaling_data,
               inits=inits3,
               n.chains=n.chains,
               n.adapt=n.adapt)
...
autocorr.plot(window(zc1, start=st+bi, end=en, thin=2)[,c("deviance")],lag.max=30)
