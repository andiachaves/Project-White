#################  STARTER CODE FOR IMPLEMENTING MODEL 1   ################
#### Fill-in the missing code (????)

setwd("C:\\Users\\MBrandt\\Dropbox\\Transfer documents\\conferences\\Bayesian Statistics\\Classwork\\Day7\\Lab7_AM_files\\")
...
jm1=jags.model("Day7_model3_STARTER_CODE.R",
               data=scaling_data,
               inits=inits1,
               n.chains=n.chains,
               n.adapt=n.adapt)  #mb
...
load.module("dic")
n.iter=2000
zc1 = coda.samples(jm1,variable.names=c("deviance","Sigma","alpha","beta","rho","sig",
                                        "Yrep","Dsum"),
                   n.iter=n.iter)
...
xyplot(window(zc1, start=st, end=en, thin=1)[,paste("alpha[",1:2,"]",sep="")],layout=c(2,2))
xyplot(window(zc1, start=st, end=en, thin=1)[,paste("beta[",1:2,"]",sep="")],layout=c(2,2))
...
th=thin(zc1)
xyplot(window(zc1, start=st, end=en, thin=1)[,c("deviance")])
xyplot(window(zc1, start=st, end=en, thin=1)[,paste("alpha[",1:2,"]",sep="")],layout=c(2,4))
xyplot(window(zc1, start=st, end=en, thin=1)[,paste("beta[",1:2,"]",sep="")],layout=c(2,4))
...
yrep2.stats=summary(window(zc1, start=st+bi, end=en, thin=n.thin)
               [,paste("Yrep[",1:scaling_data$N,",2]",sep="")])$stat #mb
yrep2.quantiles=summary(window(zc1, start=st+bi, end=en, thin=n.thin)
               [,paste("Yrep[",1:scaling_data$N,",2]",sep="")])$quantiles #mb
...
G[2] <- sum((yrep2.stats[,1]-Y[,2])^2) #mb
P[1] <- sum(yrep1.stats[,2]^2) #mb
P[2] <- sum(yrep2.stats[,2]^2) #mb
Dinf <- G + P
...
pd.star = (1/2)*((summary(window(zc1, start=st+bi, end=en, thin=n.thin)[,"deviance"])$stat[2])^2) #mb
...
#### Plots for the first variable (log(L)):
...
#### Plots for the second variable (log(M)):
plot(Y[,2],yrep2.stats[,1],ylim=c(3,5),xlim=c(3,5),pch=20,cex=.75,
