...
#MB-Create a Euclidean distance matrix that specifies the distance between all points
D=as.matrix(dist(utgrid.locs))
s2=2
phi=1.5*10^-5
Sigma=s2*exp(-D*phi) #This is the exponential covariance model
#Plot relationship between distance and covariance - exponential relationship
plot(seq(0,max(D),,20),s2*exp(-seq(0,max(D),,20)*phi),type="o",ylab="cov",xlab="distance")

#Simulate the spatially correlated error process from multivariate normal
set.seed(13)
...
#Specify how much actual data to keep - take a subset of the simulated process
...
setwd("C:\\Users\\MBrandt\\Dropbox\\Transfer documents\\conferences\\Bayesian Statistics\\Classwork\\Day9\\Day_9_Lab1\\Day_9_Lab\\")
jags.sp.m <- jags.model(file = "lab_9_jags_model.txt", data = data, inits = inits, n.chains = 1, n.adapt = 0)
...
#Black dots are where you want to predict - red dots are stations for which we have data.
...
#X[,4]=X[,2]^2  - don't forget to change p above to 4 if you add this - calculate DIC and compare among models
...
plot(variog4(temp.gd,max.dist=D.max/2))   # Plots it in all directions: this is a check for isotropy.
plot(temp.v)
...
starting <- list("phi"=10/D.max, "sigma.sq"=1)
tuning <- list("phi"=.1, "sigma.sq"=.1)
priors <- list("beta.Flat", "phi.Unif"=c(3/max.prior.range, 3/min.prior.range),"sigma.sq.IG"=c(2, 5))
cov.model <- "exponential"
