...
model
{
	...
	for (i in 1:99){  #For each patch
		for (j in 1:3) { #For each survey
			p[i,j] <- ilogit(a0 + a1*effort[i] + a2*date[i,j]) #Calculate detection probability
		}
	}
	
	for (i in 1:99){ #For each patch
		psi[i] <- ilogit(b0 + b1*ag[i] + b2*urban[i] + b3*mining[i] + b4*veg[i] + b5*size[i] + b6*isol[i])) #calculate occupancy probability
